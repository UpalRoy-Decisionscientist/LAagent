...
const origin = (process.env.PREVIEW_URL ?? "http://127.0.0.1:4173").replace(/\/$/, "");
...
try {
  await page.goto(`${origin}/?view=lesson`, { waitUntil: "domcontentloaded" });
  ...
  const title = await page.getByTestId("lesson-title").innerText();
  ...
  const steps = page.getByTestId("lesson-step");
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const snapshot = page.locator(".lg\\:col-span-7 img").first();
    await snapshot.waitFor({ state: "visible", timeout: 10_000 });
    ...
    const natural = await snapshot.evaluate((node: HTMLImageElement) => ({
